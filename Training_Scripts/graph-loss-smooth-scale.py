...
app.layout = html.Div([
    html.H1("Training and Evaluation Loss Comparison"),
    dcc.Graph(id='loss-plot'),  # Graph will be generated here
    html.Label([
        "Smoothing Window Size:",
        dcc.Slider(
            id='smoothing-slider',
            min=1,
            max=21,  # Adjust max value as needed
            step=2,  # Adjust step as needed
            value=5,  # Initial value
            marks={i: str(i) for i in range(1, 22, 2)},  # Adjust marks as needed
        )
    ]),
])
...
def update_figure(selected_window_size):
    # Apply smoothing using the selected window size from the slider
    df['Smoothed Loss'] = df.groupby(['Run ID', 'Type'])['Loss'].transform(lambda x: x.rolling(window=selected_window_size, min_periods=1, center=True).mean())
    
    # Generate the figure using Plotly Express
    fig = px.line(df, x="Step", y="Smoothed Loss", color="Configuration", line_dash="Type",
                  labels={"Smoothed Loss": "Loss", "Step": "Step"})
    
    fig.update_layout(transition_duration=500)  # Optional: adds a transition effect when updating the plot
